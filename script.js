...
(function initModal() {
  ...
  function openModal(card) {
    ...
    // Lock body scroll — iOS Safari safe method
    ...
    // Reset modal scroll position to top every open
    modal.scrollTop = 0;
  }
  ...
  function closeModal() {
    ...
    // Restore body scroll — iOS Safari safe method
  ...
  document.querySelectorAll(".project-card").forEach((card) => {
    ...
    // Use pointerup instead of click to avoid ghost-click issues on mobile
    card.addEventListener("pointerup", (e) => {
      // Ignore if it was a drag/scroll gesture (moved more than 8px)
      if (Math.abs(e.movementX) > 8 || Math.abs(e.movementY) > 8) return;
      openModal(card);
    });
  ...
  // Close button — use both click and touchend for reliability
  if (closeBtn) {
    closeBtn.addEventListener("click", (e) => { e.stopPropagation(); closeModal(); });
    closeBtn.addEventListener("touchend", (e) => { e.preventDefault(); e.stopPropagation(); closeModal(); });
  }

  // Overlay background tap to close — only when tapping directly on overlay (not modal)
